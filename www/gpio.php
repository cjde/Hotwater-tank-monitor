...
exec ("/usr/bin/python3 /var/www/ceeberry/hw/hw_ctrl2.py -g", $output, $rt );
$st = end( $output); 
// echo "get status is ",$st; 

if ($st == "1" ) {
   // if on turn it off 
   exec ("/usr/bin/python3 /var/www/ceeberry/hw/hw_ctrl2.py -f", $output, $rt );
   $st = end( $output);
   // echo "was on, now:",$st; 
   //print_r ( $output);
}
   ...
   // if off turn it on 
   exec ("/usr/bin/python3 /var/www/ceeberry/hw/hw_ctrl2.py -n", $output, $rt );
   $st = end( $output);
   // echo "was off, now:",$st; 
   //print_r ( $output);
}
